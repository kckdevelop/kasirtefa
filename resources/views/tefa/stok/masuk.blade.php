...
@section('title', 'Stok Masuk TEFa')

@section('content')
<div x-data="{ showModal: false }" class="space-y-6">
    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
            <h2 class="text-xl font-bold text-slate-900">Stok Masuk TEFa</h2>
            <p class="text-sm text-slate-500">Pencatatan riwayat pasokan dan penambahan stok produk</p>
        </div>
        <button @click="showModal = true"
            ...
            <i class="fa-solid fa-plus"></i> Catat Stok Masuk
        </button>
    </div>
    ...
    <div class="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
            ...
            <table class="w-full text-left border-collapse">
                <thead class="bg-slate-50 border-b border-slate-200">
                    <tr class="text-[11px] font-semibold text-slate-500 uppercase tracking-wider">
                        ...
                        <th class="py-3.5 px-4">Supplier / Sumber</th>
                ...
                <tbody class="divide-y divide-slate-100 text-sm">
                    ...
                    <tr class="hover:bg-slate-50">
                        ...
                        <td class="py-3.5 px-4 text-slate-600">{{ $item->supplier ?? '-' }}</td>
                    ...
                    @empty
                    <tr>
                        <td colspan="6" class="py-12 text-center text-slate-400">Belum ada catatan stok masuk.</td>
                    </tr>
                    @endforelse
                </tbody>
            </table>
        ...
        <div class="px-4 py-3 border-t border-slate-100">{{ $stokMasuk->links() }}</div>
        @endif
    </div>
    ...
    <div x-show="showModal" x-cloak class="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-4">
        <div class="bg-white rounded-2xl w-full max-w-lg shadow-2xl p-6" @click.stop>
            <div class="flex items-center justify-between mb-4">
                <h3 class="text-lg font-bold text-slate-900">Catat Stok Masuk Baru</h3>
                <button @click="showModal = false" class="text-slate-400 hover:text-slate-600"><i class="fa-solid fa-times text-xl"></i></button>
            </div>
            <form action="{{ route('tefa.stok-masuk.store') }}" method="POST" class="space-y-4">
                @csrf
                <div>
                ...
                <div class="grid grid-cols-2 gap-3">
                    <div>
                        <label class="block text-xs font-semibold text-slate-600 mb-1">Jumlah Masuk *</label>
                ...
                <div>
                    <label class="block text-xs font-semibold text-slate-600 mb-1">Supplier / Sumber Pasokan</label>
                    <input type="text" name="supplier" placeholder="Nama supplier..." class="w-full px-3 py-2 rounded-xl border border-slate-200 text-sm">
                </div>
                <div>
                    <label class="block text-xs font-semibold text-slate-600 mb-1">Catatan / Keterangan</label>
                    <textarea name="catatan" rows="2" placeholder="Catatan pasokan..." class="w-full px-3 py-2 rounded-xl border border-slate-200 text-sm resize-none"></textarea>
                </div>
                <div class="flex gap-3 justify-end pt-3 border-t border-slate-100">
                    ...
                    <button type="submit" class="px-4 py-2 bg-emerald-600 text-white font-semibold rounded-xl text-sm hover:bg-emerald-700">Simpan Stok Masuk</button>
                </div>
            </form>
        </div>
    </div>
